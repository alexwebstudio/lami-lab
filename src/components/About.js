...
/**
 * Блок «Как я работаю». `aside=true` — компактная версия для правой колонки:
 * встаёт на место скрытого портрета, чтобы там не было пустоты.
 */
function TrustBlock({ aside }) {
  return (
    <div className={`${styles.trust} ${aside ? styles.trustAside : ''}`}>
  ...
  return (
    <section className={`section ${styles.about}`} id="about" aria-labelledby="about-title">
      ...
      <Reveal className={`inner ${styles.inner}`}>
        <div className={`${styles.grid} ${showPortrait ? '' : styles.gridPlain}`}>
